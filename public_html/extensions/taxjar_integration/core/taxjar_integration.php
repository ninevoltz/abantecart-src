...
$client = TaxJar\Client::withApiKey("444c079b07d1189320532a6a8960c95e");
	        ...
	        if ( $that->request->is_POST() ) {
		        ...
		        if ( isset( $order_id )
			        ...
			        if ( $enable_calculation ) {
				        $this->calculateTax( $that, $cust_data, true, $order_totals );
			        }
			        if ( $enable_export ) {
    ...
    public function calculateTax($that, $cust_data, $commit = false, $total_data = 0, $return = false) {
        ...
        if ($api_key || $sandbox_api_key) {
            ...
            if (IS_ADMIN === true) {
                ...
                if ($route === 'r/checkout/cart/recalc_totals') {
                    $shp_cost = $that->session->data['shipping_method']['cost'];
                } else {
                    $shp_cost = $that->session->data['fc']['shipping_method']['cost'];
                }
	        ...
	        if (($to_country==='US' && $is_nexus > 0) || $to_country!=='US') {
                if ($route === 'checkout/confirm' || $route === 'checkout/guest_step_3' || $route === 'checkout/fast_checkout'
                    || $route === 'checkout/cart' || $route === 'r/checkout/cart/recalc_totals' || $route === 'r/checkout/pay/main'
                    || $route === 'r/checkout/fast_checkout_summary' || $that->session->data['is_admin'] === '1') {
                    unset($that->session->data['is_admin']);
                    $zipcode_exist = $that->cache->pull($cache_zipcode);
                    $total_exist = $that->cache->pull($cache_total);
                    if (!$zipcode_exist || $zipcode_exist !== $to_zipcode) {
                        $that->cache->push($cache_zipcode, $to_zipcode);
                        $new_zipcode = true;
                    } else {
                        $new_zipcode = false;
                    }
                    ...
                    if (!$total_exist || $total_exist !== $total) {
                        $that->cache->push($cache_total, $total);
                        $new_total = true;
                    } else {
                        $new_total = false;
                    }
	                if (($new_zipcode === true && $new_total === true) || $new_zipcode === true || $new_total === true) {
                        if ($lines) {
                            try {
                                ...
                                $that->cache->push($taxjar_tax_rate_cache, $getTaxRequest->amount_to_collect);
            ...
            if ($route ==='checkout/confirm' || $route==='checkout/guest_step_3' || $route === 'checkout/fast_checkout' || $route === 'r/checkout/fast_checkout_summary'
                    || $route === 'checkout/cart' || $route === 'r/checkout/pay/main') {
            	return $that->cache->pull($taxjar_tax_rate_cache);
            } else {
	            return $tax_rate;
            }
        }
