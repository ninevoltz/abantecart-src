...
{
    public $error;
    protected $filterByType = 'extensions';
    protected $mp_category_map = [
        'extensions'   => 0,
        'payment'      => 73,
        'shipping'     => 73,
        'template'     => 66,
    ...
    public function __construct($registry, $instance_id, $controller, $parent_controller = '')
    {
        ...
        $this->filterByType = $this->session->data['extension_filter'];
        if (!in_array($this->filterByType, array_keys($this->mp_category_map))) {
            $this->filterByType = 'extensions';
        }
    }
    ...
    public function main()
    {
        ...
        $mpMdl = $this->loadModel('tool/mp_api');
        $mp_token = $this->config->get('mp_token');
        if ($mp_token) {
            $this->view->assign('mp_connected', true);
            $this->session->data['ready_to_install'] = $mpMdl->getMyExtensions($mp_token);
        }
        ...
        $this->document->addBreadcrumb(
            [
                'href'      => $this->html->getSecureURL(
                    'extension/extensions/' . $this->filterByType
                ),
        ...
        $mp_params = [
            'rt' => 'account/authenticate',
            'return_url' => $return_url,
            'store_id' => UNIQUE_ID,
            'store_url' => HTTPS_SERVER,
            'store_version' => VERSION
        ];
        $this->view->assign('amp_connect_url', $mpMdl->getMPURL() . '?'.http_build_query($mp_params));
    ...
    {
        $this->filterByType = $this->session->data['extension_filter'] = 'extensions';
        $this->main();
    }

    public function payment()
    {
        $this->filterByType = $this->session->data['extension_filter'] = 'payment';
        $this->main();
    }

    public function shipping()
    {
        $this->filterByType = $this->session->data['extension_filter'] = 'shipping';
        $this->main();
    }

    public function template()
    {
        $this->filterByType = $this->session->data['extension_filter'] = 'template';
        $this->main();
    }

    public function language()
    {
        $this->filterByType = $this->session->data['extension_filter'] = 'language';
        $this->main();
    }

    public function tax()
    {
        $this->filterByType = $this->session->data['extension_filter'] = 'tax';
        $this->main();
    }
    ...
    public function edit()
    {
        ...
        $this->document->addBreadcrumb(
            [
                'href'      => $this->html->getSecureURL( 'extension/extensions/'. $this->filterByType ),
                'text'      => $this->language->get('heading_title'),
                'separator' => ' :: ',
            ]
        );
        ...
        $store_id = $this->request->get_or_post('store_id') ?? (int)$this->session->data['current_store_id'];
        ...
        foreach ($settings as $item) {
            ...
            if ($item['note']) {
                ...
                $langDefinitionKey = str_replace($extension . '_', 'text_', $data['name']);
                if (in_array($data['name'], [$extension . '_status', $extension . '_sort_order'])) {
                    $noteText = $this->language->get($langDefinitionKey, 'extension/extensions');
                } else {
                    $noteText = $this->language->get($data['name'], '', true);
                }
                // if text definition not found - seek it in default settings definitions
                if ($noteText == $data['name']) {
                    $noteText = $this->language->get($langDefinitionKey, 'extension/extensions');
                    //add length and weight units to fields descriptions
                    if (str_ends_with($data['name'], '_volume')) {
                        ...
                        if ($lengthClass['title']) {
                            $noteText = $this->language->getAndReplace(
                                $langDefinitionKey,
                    ...
                    if (str_ends_with($data['name'], '_weight')) {
                    ...
                    if ($noteText == $langDefinitionKey) {
                        $noteText = $this->language->get(
                            $langDefinitionKey . '_' . $this->data['extension_info']['type'],
                            '',
                            true
                        );
                    }
            ...
            $data['style'] = $item['style'] ?: $data['style'];
            $data['attr'] = $item['attr'] ?: $data['attr'];
            $data['readonly'] = $item['readonly'] ?? $data['readonly'];
        ...
        if ($this->request->is_POST() && $this->_validateSettings($extension, $store_id)) {
            ...
            $this->extension_manager->editSetting($extension, $post);
            $this->cache->remove('settings');
        ...
        if (in_array($extension, $missing_extensions)) {
            $this->session->data['error'] = $this->language->getAndReplace(
                'text_missing_extension',
                replaces: $extension
            );
            redirect($this->html->getSecureURL('extension/extensions'));
        }
        ...
        if ($ext->getConfig('additional_settings')) {
            ...
            if ($store_id) {
                ...
                $btn_param['link'] = $store_info['config_url']
                    . '?'
                    . http_build_query(
                        [
                            's'  => ADMIN_PATH,
                ...
                $btn_param['onclick'] = 'onclick="return confirm(\'' . js_encode($this->language->get('additional_settings_confirm')) . '\');"';
            }
    ...
    public function install()
    {
        ...
        if (!$this->user->canModify('extension/extensions')) {
            ...
            redirect($this->html->getSecureURL('extension/extensions/' . $this->filterByType));
        } else {
            ...
            if (!$validate) {
                ...
                if ($this->extension_manager->isExtensionInstalled($this->request->get['extension'])) {
                    ...
                    $url = $this->html->getSecureURL('extension/extensions/' . $this->filterByType);
                }
                redirect($url);
            }
            ...
            if ($config === false) {
                $filename = DIR_EXT . str_replace('../', '', $this->request->get['extension']) . DS . 'config.xml';
    ...
    public function delete()
    {
        ...
        if (!$this->user->canModify('extension/extensions')) {
            ...
            //extensions saved in DB but missing files
            $missing_extensions = $this->extensions->getMissingExtensions();
